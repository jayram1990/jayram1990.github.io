...
app.get("/", (req, res) => {
    ...
    res.write('<h1>Jayaraman- SAC Chat Bot with SAP CAI </h1>');
    res.end();
});
...
function onMessage(message) {
    ...
    if (sessionid === "" || typeof sessionid === "undefined") {
        message.addReply([{
            type: 'text',
            content: 'Please enter the chat id, it should be displayed near my area.'
        }])
        message.reply()
            .then(res => console.log('message sent, Thank you.'))
    } else {
        if (message.conversation.memory.hasOwnProperty("user")) {
            ...
            if (message.nlp.entities.hasOwnProperty("uid")) {
                message.addReply([{
                    type: 'text',
                    content: 'Thank you, You have entered the chat ID. Now you can ask us (CMI MMT)'
                }])
                message.reply()
                    .then(res => console.log('message sent jay'))
            }
        } else {
            //no memory
